...
- ✅ **全局热键** — F9暂停/F10跳过/F12紧急停止，后台静默运行
- ✅ **策略层** — 复合key索引 + 模糊匹配 + 三级回退，精准答案定位
- ✅ **远程配置** — 版本检查 + 强制更新 + 远程开关 + 公告推送，多源容灾
...
2. **Tab 2 📚 离线试卷浏览器**：左侧选择试卷，右侧逐节查看题目和答案
...
└──────┬───────────────┬───────────┘
       ...
       ▼               ▼
┌──────────────┐  ┌───────────────┐  ┌──────────────┐
│ ets_auto.py  │  │ ets_word_pk.py│  │ ets_remote.py│
│ 套卷自动答题  │  │ 单词PK自动答题 │  │ 远程配置      │
│ - setPCChoose2│  │ - 四级匹配策略 │  │ - 版本检查    │
│ - 原生Setter │  │ - 派生词生成   │  │ - 强制更新    │
│ - 本地JSON   │  │ - 双向自学习   │  │ - 远程开关    │
│ - RW读写同步  │  │               │  │ - 公告推送    │
└──────┬───────┘  └───────────────┘  └──────────────┘
       ...
       ▼
┌──────────────┐  ┌───────────────┐  ┌──────────────┐
│ ets_gui.py   │  │ ets_parser.py │  │ ets_hotkey.py │
│ 图形界面启动器 │  │ 离线试卷浏览器 │  │ 全局热键      │
│ - 模式/端口   │  │ - 选择题标红   │  │ - F9暂停/恢复 │
│ - 实时日志    │  │ - 填空标准答案 │  │ - F10跳过     │
│ - 远程配置集成 │  │ - 口语可接受答案│  │ - F12停止     │
└──────────────┘  └───────────────┘  └──────────────┘
       │
       ▼
┌──────────────┐
...
└── src/auto/
    ...
    ├── ets_auto.py                # ★ 套卷自动答题 (含RW读写同步)
    ├── ets_word_pk.py            # ★ 单词PK自动答题
    ├── ets_strategy.py           # ★ 策略层 (索引+模糊匹配+回退)
    ├── ets_hotkey.py             # ★ 全局热键 (F9/F10/F12)
    ├── ets_gui.py                # ★ 图形界面 (CustomTkinter + 远程配置)
    ├── ets_remote.py             # ★ 远程配置 (版本检查/强制更新/公告)
...
* [x] **热键支持** —— 全局快捷键 F9暂停/F10跳过/F12停止。 ✅ v0.5.0
* [x] **策略层** —— 复合key索引+模糊匹配+三级回退。 ✅ v0.5.0
* [x] **远程配置** —— 版本检查+强制更新+远程开关+公告推送。 ✅ v0.6.0
* [x] **读写同步模式** —— 支持 ETS 读写同步题型自动答题。 ✅ v0.5.2
* [ ] **作业模式实战测试** —— 真实作业场景端到端验证
* [ ] **试卷浏览器导出** —— 支持打印/导出为 PDF
